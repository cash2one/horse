...
{
	char * Ret=strncpy(strDest,strSource,count);
	//strDest[count-1]=0;
	return Ret;
}

inline wchar_t *strncpy_0( wchar_t *strDest, const wchar_t *strSource, size_t count )
{
...
public:
	...
	void Format(const wchar_t * pFormat,...)
	{
		...
		Resize(Len,false);
		vswprintf(m_pBuffer,pFormat,vl);
		m_StringLength=Len;
		va_end( vl);
	}
	void MakeUpper()
...
template<>
void CEasyStringT<char>::SetString(const wchar_t * pStr,size_t Size)
{
	if(pStr&&Size>0)
	{
...
template<>
void CEasyStringT<char>::AppendString(const wchar_t * pStr,size_t Size)
{
	if(Size>0)
	{	
		size_t AnsiSize=UnicodeToAnsi(pStr,Size,NULL,0);
...
template<>
void CEasyStringT<wchar_t>::SetString(const char * pStr,size_t Size)
{
	if(pStr&&Size>0)
	{
...
template<>
void CEasyStringT<wchar_t>::AppendString(const char * pStr,size_t Size)
{
	if(pStr&&Size>0)
	{
		size_t UnicodeSize=AnsiToUnicode(pStr,Size,NULL,0);
...
template<>
void CEasyStringT<char>::Format(const wchar_t * pFormat,...)
{
	va_list	vl;		
	va_start(vl,pFormat);
	size_t Len=_vscwprintf(pFormat,vl);
	wchar_t * pBuffer=new wchar_t [Len];	
	vswprintf(pBuffer,pFormat,vl);
...
template<>
void CEasyStringT<wchar_t>::Format(const char * pFormat,...)
{
	va_list	vl;		
	va_start(vl,pFormat);
	size_t Len=_vscprintf(pFormat,vl);
	char * pBuffer=new char [Len];	
	vsprintf(pBuffer,pFormat,vl);
	SetString(pBuffer,Len);
	SAFE_DELETE_ARRAY(pBuffer);
	va_end( vl);
}
...
template<>
void CEasyStringT<char>::MakeUpper()
{
	_strupr(m_pBuffer);
}

template<>
void CEasyStringT<wchar_t>::MakeUpper()
{
	_wcsupr(m_pBuffer);
}

template<>
void CEasyStringT<char>::MakeLower()
{
	_strlwr(m_pBuffer);
}

template<>
void CEasyStringT<wchar_t>::MakeLower()
{
	_wcslwr(m_pBuffer);
}
