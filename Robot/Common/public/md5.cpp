...
void BinaryToString(const BYTE* pBinary, char* sDisplayString, int nBinaryLen)
{
	BYTE nTmp = 0;
	for(int i=0; i<nBinaryLen; i++)
	{
		nTmp = (pBinary[i] & 0xF0)>>4;
		if(nTmp>=0x00 && nTmp<=0x09)
		{
			sDisplayString[i<<1] = '0'+ nTmp;
		}
		else if(nTmp>=0x0A && nTmp<=0x0F)
		{
			sDisplayString[i<<1] = 'A'- 0x0A + nTmp;
		}
		else
		{
			//_ASSERT(false);
		...
		if(nTmp>=0x00 && nTmp<=0x09)
		{
			sDisplayString[(i<<1)+1] = '0'+ nTmp;
		}
		else if(nTmp>=0x0A && nTmp<=0x0F)
		{
			sDisplayString[(i<<1)+1] = 'A'- 0x0A + nTmp;
		}
		else
		{
			//_ASSERT(false);
