...
int CMySQLRecordSet::Init(MYSQL_RES_HANDLE hResults)
{
	if(hResults==NULL)
	...
	{		
		strncpy_0(m_pColumnInfos[i].Name,pFields[i].name,sizeof(m_pColumnInfos[i].Name) );
...
int CMySQLRecordSet::GetIndexByColumnName(LPCTSTR Name)
{
	...
	{
		if(strnicmp(m_pColumnInfos[i].Name,Name,MAX_COLUMN_NAME)==0)
			return i;
	}
	return -1;
}
...
int CMySQLRecordSet::GetParamCount()
{
	return 0;
}

IDBValue& CMySQLRecordSet::GetParam(int Index)
{
	return m_EmptyValue;
}

IDBValue& CMySQLRecordSet::GetField(int Index)
{
...
IDBValue& CMySQLRecordSet::GetField(LPCTSTR Name)
{
...
int CMySQLRecordSet::MoveTo(int Index)
{
	if(m_hResults)
	{
		int num = GetRecordCount();
		if( num >=1 && num <= 9999999 && Index>=0 && Index< num )
		{		
			if(m_CurRow!=Index)
			{
				m_CurRow=Index;
...
bool CMySQLRecordSet::IsBOF()
{
	if(m_hResults)
	{
		return m_CurRow==GetRecordCount()-1;
	}
	else
		return true;
}
...
int CMySQLRecordSet::FetchRow()
{
	...
	if(RowData&&ValueLen)
	{
		...
		{
			m_RowBuffer[j].SetValue(m_pColumnInfos[j],RowData[j],ValueLen[j]);		
		}
		return DBERR_SUCCEED;
	}
	else
