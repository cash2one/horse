...
MySQLConnection::MySQLConnection(boost::lockfree::queue<SQLOperation*>* queue, MySQLConnectionInfo& connInfo) :
m_reconnecting(false),
m_prepareError(false),
m_queue(queue),
m_Mysql(NULL),
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    //if (!ret)
        // TC_LOG_ERROR(LOG_FILTER_SQL, "Could not fetch prepared statement %u on database `%s`, connection type: %s.",index, m_connectionInfo.database.c_str(), (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous" : "synchronous");
...
//             boost::this_thread::sleep(boost::posix_time::seconds(3));                           // Sleep 3 seconds
