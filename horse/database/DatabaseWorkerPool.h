...
class DatabaseWorkerPool
{
    public:
        /* Activity state */
        DatabaseWorkerPool()
 			: _queue(new boost::lockfree::queue<SQLOperation*>(200)) // todo
        {
...
//             else
                // TC_LOG_ERROR(LOG_FILTER_SQL_DRIVER, "DatabasePool %s NOT opened. There were errors opening the MySQL connections. Check your SQLDriverLogFile " "for specific errors.", GetDatabaseName());
            return res;
        }
...
//             _queue->queue()->close();
            ...
            // TC_LOG_INFO(LOG_FILTER_SQL_DRIVER, "Asynchronous connections on DatabasePool '%s' terminated. Proceeding with synchronous connections.",  GetDatabaseName());
            ...
            //! Deletes the boost::lockfree::queue object and its underlying ACE_Message_Queue
            delete _queue;
...
// 			_ios.post(*op,)
    ...
    private:
        ...
        boost::lockfree::queue<SQLOperation*>*         _queue;             //! Queue shared by async worker threads.
